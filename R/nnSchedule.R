...
.nnInferSched <- function(env, nW = NA_integer_, hasTrained = FALSE) {
  ...
  .s <- list(
    ...
    ##
    ## `kinetic` ships off for a reason of its own on top of that one: it is the
    ## only penalty here that changes the dynamics the SOLVER has to integrate
    ## rather than only steering the optimizer, so a default would quietly change
    ## the model rather than only the fit.  Turn it on when the augmented solve
    ## is slow or the learned term looks stiffer than the data justifies.
